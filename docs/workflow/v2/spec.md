...
**Follow-up fixes after a real audit run surfaced genuine bugs, not a business-context gap:** a first cookie-policy audit came back scoring 5/10 with issues that turned out to split into two different causes. (1) Real bugs in the generator: policy generation was capped at 2048 tokens, truncating longer policies mid-sentence, and the prompt never explicitly asked for a contact section, an update-notice, or cross-links to other policies - all fixed (policy pieces now get 4096 tokens, and the prompt requires all three). (2) A real, separate compliance gap: the existing "Cookie Preferences" page only ever linked out to browser settings and third-party opt-out tools - there was no actual on-site accept/reject/manage mechanism, which is what PECR requires for non-essential cookies. `inc/cookie-consent.php` + `assets/js/cookie-consent.js` now provide one: a real banner (Accept All / Reject Non-Essential / Manage Preferences) that remembers the choice for 6 months and exposes `window.wookieeConsent` for any future analytics/marketing script to check before firing (none are wired into the theme yet, so there's nothing to gate today - this is the infrastructure ahead of that). The existing live "Cookie Preferences" page is patched in place (self-healing on `init`, like the theme's other starter-content repairs) to add a real "Manage cookie preferences" button. The Cookie Policy prompt now describes this mechanism accurately from a shared fact string, rather than omitting it or inventing one that didn't exist.

Also added: an **"Apply these fixes to the draft"** button under the audit report - feeds the audit's own itemised issues back into a rewrite of the same draft page (still Draft-only, still requires a manual Publish), instead of the admin re-typing what the AI already told them to fix.
...
**Update after a real search run showed the actual problem:** CJ's own keyword search returns a lot of noise - a live "UK home decor products" search returned a leg massager filed under "Home Office Storage" alongside genuinely relevant items, and every raw title is written in cross-border-seller style ("Cross-border Aluminum Material Serpentine Night Light..."), which is exactly the kind of keyword-stuffed, unclear title GMC policy flags. Two additions close this: (1) `wookiee_ai_clean_supplier_product()` runs every import's raw title/description through the LLM to produce a clean, accurate, GMC-appropriate title/description/category - never inventing facts, only rewriting presentation of the same real product - and separately judges whether the item actually fits the store's niche; (2) bulk import (checkboxes + "Import selected as drafts", capped at 10 per batch) auto-skips low-fit items since that's exactly where curating out catalog noise matters most, while a single explicit "Import as draft" click still honors what the admin picked, attaching the AI's fit judgement as an advisory note instead of blocking it.
