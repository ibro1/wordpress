...
/**
 * The accepted-payment marks.
 *
 * Lived only in the site footer, so they were absent from the one page where
 * they actually do some work: a customer weighing up whether to hand over a
 * card is at checkout, not scrolling the bottom of the homepage. Shared so
 * the two cannot drift - which is how the earlier copy came to be white text
 * in one place and real marks in another.
 */
function wookiee_render_payment_marks( $class = 'footer-payments' ) {
	?>
	<div class="<?php echo esc_attr( $class ); ?>">
				...
				<div class="payment-icon-wrapper" title="PayPal">
					<?php
					/*
					 * The double-P monogram rather than the two thin outlined
					 * P's this carried before: three tones, with the darker
					 * wedge where the front P crosses the back one. Drawn as
					 * plain paths - no <defs>, no clipPath, no ids - because
					 * this markup renders on the site footer and again in the
					 * cart/checkout footer, and duplicate SVG ids on one
					 * document resolve to whichever came first.
					 */
					?>
					<svg viewBox="0 0 48 30" role="img" aria-label="PayPal" focusable="false">
						<path fill="#003087" fill-rule="evenodd" d="M13.4 2.5h9c5.3 0 8.4 3.3 7.6 8.4-.8 5.2-4.7 8.1-9.8 8.1h-3.3l-1.3 8.5H9.8l3.6-25Zm3.5 4.8-1 6.4h2.9c2.3 0 3.8-1.2 4.1-3.3.3-2-.8-3.1-2.9-3.1h-3.1Z"/>
						<path fill="#009CDE" fill-rule="evenodd" d="M20.9 7.5h9c5.3 0 8.4 3.3 7.6 8.4-.8 5.2-4.7 8.1-9.8 8.1h-3.3l-.5 3.5h-5.1l2.1-20Zm3.5 4.8-1 6.4h2.9c2.3 0 3.8-1.2 4.1-3.3.3-2-.8-3.1-2.9-3.1h-3.1Z"/>
						<path fill="#012169" d="M20.9 7.5h4.8c-.3 4.3-3 6.9-7.4 7.5l-1.4 8.6h-1.9l2.4-16.1h3.5Z"/>
					</svg>
				</div>
...
function wookiee_render_checkout_trust_footer() {
    ...
    <?php
    /*
     * The payment marks belong here more than in the site footer. Someone
     * about to enter a card wants to know it will be accepted; someone
     * scrolling the bottom of the homepage does not. They stay in both, but
     * this is the placement that does the work.
     */
    wookiee_render_payment_marks( 'checkout-payments' );
}
