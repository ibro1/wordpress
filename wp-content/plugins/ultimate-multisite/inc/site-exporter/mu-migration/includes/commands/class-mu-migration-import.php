...
class ImportCommand extends MUMigrationBase {
	...
	public function users($args = [], $assoc_args = [], $verbose = true) {
		...
		if ( false !== $input_file_handler ) {
			...
			while ( false !== ($data = fgetcsv($input_file_handler, 0, $delimiter, '"', '\\')) ) {
				...
				if ( ! $user_exists ) {
					...
					if ( ! is_wp_error($new_id) ) {
						...
						/**
						 * Ignores it is already documented in
						 * inc/mu-migration/includes/commands/class-mu-migration-export.php:335.
						 *
						 * @ignore
						 */
						$custom_user_data = apply_filters('mu_migration/export/user/data', [], $user);
						...
						$this->warning(
							sprintf(
								__('An error has occurred when inserting %1$s: %2$s.', 'mu-migration'),
								$user_data['user_login'],
					...
					$this->warning(
						sprintf(
							__('%1$s exists, using his ID (%2$d)...', 'mu-migration'),
							$user_data['user_login'],
							$user_exists
						),
						$verbose
			...
			$this->success(
				sprintf(
					__('%1$d users have been imported and %2$d users already existed', 'mu-migration'),
					absint($count),
	...
	private function move_and_activate_plugins($plugins_dir, $plugins, $blog_plugins, $network_plugins) {
		if ( file_exists($plugins_dir) ) {
			...
			foreach ( $plugins as $plugin_name => $plugin ) {
				...
				} elseif ( $check_plugins && in_array($plugin_name, $network_plugins, true) ) {
