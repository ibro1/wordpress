(function($) {
		...
		methods: {
			...
			apply_coupon() {
				...
				if (this.coupon) {
					...
					const coupon_type = this.type;

					const coupon_value = this.value;

					const applies_to_setup_fee = this.applies_to_setup_fee;
					const setup_fee_discount_value = this.setup_fee_discount_value;
					const setup_fee_discount_type = this.setup_fee_discount_type;
					const allowed_plans = $.parseJSON(this.allowed_plans);
					const allowed_freqs = $.parseJSON(this.allowed_freqs);
					...
					setTimeout( function() {
						$('.wu-plan').each(function() {
							...
							if (typeof allowed_plans === 'object'){
								...
								for (const each_plan of allowed_plans) {
							...
							if (typeof allowed_freqs === 'object'){
								...
								for (const each_freq of allowed_freqs) {
							...
							const old_yearly_value = old_price * jQuery('#wu_plan_freq').val();

							let new_yearly_value = 0;

							const old_setupfee = $(this).find('.pricing-table-setupfee') ? $(this).find('.pricing-table-setupfee').attr('data-value') : 0;
							...
							} else if (jQuery('#wu_plan_freq').val() > 1){
							...
							if (new_price > 0) {
								...
								const plan_price = $(this).find('.plan-price');
								plan_price.html(' ' + wu_coupon_data.free_text);
								const hagacinco = $(this).find('h5');
					...
					$('.wu-plan').each(function() {
						...
						const price = $(this).data('price-' + $('#wu_plan_freq').val());
	...
	function wu_set_yearly_value(list, value) {
		...
		const current_freq = jQuery('#wu_plan_freq').val();

		const string = jQuery(list).find('.total-price.total-price-' + current_freq).html();

		if (string) {

			const parts = string.split(',');

			const result = accounting.formatMoney(parseFloat(value)) + ', ' + parts[ 1 ];
